...
def optimize_MLE_merge_guess(guess1):
	...
	K8 = matern(guess1[12],1.,numpy.absolute(guess1[13]),new_dists_t_1) * s_matrix_4
	
	K9 = matern(guess1[14],1.,numpy.absolute(guess1[15]),new_dists_t_1) * s_matrix_5
	
	WN = WHE_NSE(guess1[16],guess1[17],guess1[18],guess1[19],t_matrix_1,noise_mat_1,s_matrix_9,s_matrix_10,s_matrix_11,s_matrix_12)
	
	K_1 = K1 + K2 + K3 + K5 + K6 + K7 + K8 + K9 + WN # + K4 
...
guess_file = wkspc + 'modes_K_reg.csv'	
...
print ("run Medford 'local'")

t1 = float(time.time())

index_to_change = 0	

positive_indices = numpy.array([0,1,2,3,4,5,7,9,11,12,13,14,15,16,17,18,19])
