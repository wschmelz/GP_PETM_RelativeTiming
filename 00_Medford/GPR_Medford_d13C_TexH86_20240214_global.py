...
def optimize_MLE_merge_guess(guess1):
	...
	#K4 = matern(guess1[2],3.,numpy.absolute(guess1[3]),new_dists_t_1) * s_matrix_3
	
	K5 = linear1(guess1[4],guess1[5],new_mults_t_1,s_matrix_1)
	
	K6 = linear1(guess1[6],guess1[7],new_mults_t_1,s_matrix_2)
	
	K7 = linear1(guess1[8],guess1[9],new_mults_t_1,s_matrix_3)
	...
	WN = WHE_NSE(guess1[10],guess1[11],guess1[12],guess1[13],t_matrix_1,noise_mat_1,s_matrix_9,s_matrix_10,s_matrix_11,s_matrix_12)
	
	K_1 = K1 + K3 + K6 + K7 + WN + K5 
...
print ("run Medford 'global'")

t1 = float(time.time())

index_to_change = 0	

positive_indices = numpy.array([0,1,2,3,5,7,9,10,11,12,13])
...
for n in range(MCMC_iters):
	...
	while new_alpha[1] > 10.0:
		new_alpha[1] = numpy.absolute(numpy.random.normal(loc = old_alpha[1], scale = stepsizes[1]))
	while new_alpha[3] > 10.0:
		new_alpha[3] = numpy.absolute(numpy.random.normal(loc = old_alpha[3], scale = stepsizes[3]))		
